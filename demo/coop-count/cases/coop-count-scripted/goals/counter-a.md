...
context:
  ...
  min_span_age: 16
script:
  # The tape this job plays under `--machine scripted`: the counting half of the body
  # below written out -- ten numbers, the three commands that end a turn, and then the
  # sleep that waits for the peer. A model works the numbers out from the status line;
  # a tape cannot, so this one is only right for the schedule in `events.jsonl`.
  #
  # First turn, from zero. The status line reads nothing yet and the target is ten.
  - emit: "say 1;"
  - emit: "say 2;"
  - emit: "say 3;"
  - emit: "say 4;"
  ...
  - emit: "read stdin;"
  # Woken by counter-b handing back at sixty, so the target is seventy. The live case
  # never ends and this one has to, so the tape stops halfway up this turn rather than
  # counting on: `exit;` leaves counter-b asleep on a pipe nobody will write to again,
  # and the run goes quiet.
  - emit: "say 61;"
  - emit: "say 62;"
  - emit: "say 63;"
  - emit: "say 64;"
