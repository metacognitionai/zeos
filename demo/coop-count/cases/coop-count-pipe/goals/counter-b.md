...
# status region saying `10` and a token it had never consumed. A job's first act is chosen
# by a sampler, so waiting has to be a state the kernel puts it in rather than an
# instruction in its prose. The write that releases it is consumed and injected, which is
# why the body below opens on a `say` and never on a `read`. See TUTORIAL.md 4.2.
pinned: true
reads:
